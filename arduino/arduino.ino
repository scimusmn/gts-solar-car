...
const int EasyTimePot =
    A0;  // potentiometer used to set easy race time for win.
const int HardTimePot = A2;  // potentiometer used to set had race time for win.

const int NUM_LEDS = 48;
Adafruit_NeoPixel strip =
    Adafruit_NeoPixel(NUM_LEDS, LEDRingPIN, NEO_RGB + NEO_KHZ800);

int lapsToWin = 1, rainbow = 0;  // The number of laps to be completed for win.
long timeToRace =
    20000;  // in millisec, time you have to complete the designated # of laps.

bool isResetting = 1, isRacing = 0, hasWon = 1, ledState = 0,
     encoderLastRead = 0;  // flags
int lapCounter, currentSpeed = 0, targetSpeed, position;
int paceCarFirstLED;  // index number for neopixel strip to create 4 pixel pace
                      // car.
...
void loop() {
  ...
  if (isResetting) {
    if (hasWon) {  // create rainbow pattern.
  ...
  if (currentMillis - previousRampMillis >=
      5) {  // every 5 ms check if speed should increase
  ...
  if (!hallLineState && !hallLinePrevState) {  // still on sensor hallLine
    if (isResetting && timeElapsed > 4000) {
      isResetting = 0;
      allLEDS(0, 0, 0);
      strip.setPixelColor(45, 139, 0,
                          139);  // set color purple to create virtual pace car
  ...
  if (isResetting == 1 &&
      timeElapsed > 2000) {  // 2 sec delay after Solar lamp is off, then kick
                             // in motor reset.
  ...
  if (isResetting == 0 &&
      isRacing == 0) {  // ready to race, waiting button press
...
void waitToStart() {  // wait for user to press start button.
  digitalWrite(EasyBtnLED,
               HIGH);  // light Start button telling visitor they can play!
  digitalWrite(MedBtnLED,
               HIGH);  // light Start button telling visitor they can play!
  digitalWrite(HardBtnLED,
               HIGH);  // light Start button telling visitor they can play!
  bool is_waiting = true;
  long easyTime = analogRead(EasyTimePot);
  easyTime = map(
      easyTime, 0, 1023, 30000,
      90000);  // convert to milliseconds for race time (10 sec thru 61.15 sec)
  long hardTime = analogRead(HardTimePot);
  hardTime = map(
      hardTime, 0, 1023, 5000,
      30000);  // convert to milliseconds for race time (10 sec thru 61.15 sec)
  ...
  previousTimingMillis = startMillis =
      millis();  // record start time, initialize perviousTimingMillis for new
                 // race.
  lapCounter = 0;
}
